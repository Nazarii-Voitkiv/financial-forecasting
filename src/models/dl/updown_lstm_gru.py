...
EVAL_YEAR = 2025
METRICS_CSV = os.path.join(REPORT_DIR, f"metrics_updown_{EVAL_YEAR}_rnn.csv")
...
def train_model(cell: Literal["lstm", "gru"], cfg: TrainingConfig, feature_cfg: FeatureConfig) -> tuple[pd.Series, pd.Series, dict, pd.Series]:
...
def save_results(model_name: str, y_pred: pd.Series, y_true: pd.Series, metrics: dict, close: pd.Series) -> None:
    ...
    out_png = os.path.join(REPORT_DIR, f"updown_{EVAL_YEAR}_{model_name}.png")
    plot_predictions(close, y_pred, y_true, model_name, out_png)
...
def main():
    ...
    for cell in ("lstm", "gru"):
        ...
        y_pred, y_true, metrics, close = train_model(cell, cfg, feature_cfg)
        save_results(cell, y_pred, y_true, metrics, close)
        print(
            f"{cell.upper()} — Dokładność: {metrics['accuracy']*100:.2f}%"
        )
